...
function SongDetail({ songID }) {
    ...
    useEffect(async () => {
        let res = await axios.get("http://localhost:8000/api/ratings/");
...
const handleSubmit = async (event) => {
        ...
        if (!isError) {  
            axios.put(`http://localhost:8000/api/ratings/${ratingObjectID}/`, {rating : input_rating, song : songID, username : 1})
                .then(res => {
                    if (input_rating){
